 ...
 *
 *  - `posM`: position radius (m). Matches `PARKING_BENCH_TUNING.arriveRadius`
 *    on the runner side so the tracker stops trying to refine its pose
 *    once it's inside the same radius the goal-check uses.
 *  - `hdgRad`: heading tolerance (rad). Matches the planner's
 *    `plannerGoalHeadingTol` so the search and the goal-check agree.
...
export interface ParkingGoalCheck {
  ...
  hdgOk: boolean;
  spdOk: boolean;
  /** Measured terminal errors (signed/abs as labelled). */
  posM: number;
  hdgRad: number;
  spdMS: number;
}
 ...
 * (or vice versa), which is precisely the bug we're solving by
 * centralising the criterion.
 */
