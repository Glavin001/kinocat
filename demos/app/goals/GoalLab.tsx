...
export default function GoalLab() {
  ...
  useEffect(() => {
    ...
    function load(id: string) {
      ...
      if (path.length > 0) content.add(createPlanPathHelper(densePath(), { color: 0x66ffaa, y: 0.12 }));
    ...
    // Heading-aware cubic Hermite between two plan nodes. Tangents point along
    // TRAVEL (signed by gear), so a long analytic-shot edge bends as a curve
    // instead of a straight chord, and reverse (parking) curves the right way.
    function hermite(a: CarKinematicState, c: CarKinematicState, u: number): { x: number; z: number } {
      const chord = Math.hypot(c.x - a.x, c.z - a.z) || 1;
      const sa = Math.sign(a.speed) || 1;
      const sc = Math.sign(c.speed) || 1;
      const m0x = Math.cos(a.heading) * sa * chord;
      const m0z = Math.sin(a.heading) * sa * chord;
      const m1x = Math.cos(c.heading) * sc * chord;
      const m1z = Math.sin(c.heading) * sc * chord;
      const u2 = u * u;
      const u3 = u2 * u;
    ...
    function poseAt(tSim: number): CarKinematicState {
      ...
      for (let i = 0; i + 1 < path.length; i++) {
        ...
        if (tSim >= a.t && tSim <= c.t) {
          ...
          const p = hermite(a, c, u);
          // Heading: shortest-arc interpolation of the real vehicle headings —
          // correct for forward AND reverse, no stale-heading lag.
          const dh = Math.atan2(Math.sin(c.heading - a.heading), Math.cos(c.heading - a.heading));
          return { x: p.x, z: p.z, heading: a.heading + dh * u, speed: a.speed, t: tSim };
        }
      }
      return path[path.length - 1]!;
    }

    // Dense, smooth polyline of the plan for the green path overlay.
    function densePath(): CarKinematicState[] {
