...
describe('ramp + affordance demo', () => {
  ...
  it('rampHeightSampler is 0 off-ramp, height at the crest, drops past the crest', () => {
    ...
    // Past the crest the back-slope skirt (~2.5 m) ramps the height down to
    // 0 over the lip the car launches off; well beyond it the sampler is 0
    // again. (A vertical lip / triangle would intermittently trap Rapier's
    // WASM raycaster, so the implementation deliberately smooths it.)
    expect(sampler(r.base.x + r.length / 2 + 5, 0)).toBe(0);
